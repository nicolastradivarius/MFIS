...
pred cambioLuz [t,t1: Time,s:Semaforo] {
	some c, c1: Color | (c -> t) in s.situacion and 
	(c1-> t1) in s.situacion and (c-> c1) in secuenciaColor
}

-- relacion entre las estampillas de tiempo
fact traces{ 
	(all s:Semaforo | Rojo -> ord/first in s.situacion) and
	(all s:Semaforo, t:Time - ord/last | let t1= t.next | cambioLuz[t,t1,s])
}

-- el semáforo comienza en color Rojo
pred init [t:Time,s: Semaforo] {
